...
; returns true or false if key is special form
;(define (special-form-exist? key)
;  (if (not (lookup-action key))
;      #t
;      #f))
...
; prevent crash from special form name
...
; return #t if prim procedure exists, else #f
;(define (primitive-proc-exist? exp)
;  (define (scan names actions)
;    (cond ((null? names) #f)
;          ((and (eq? names exp) (eq? (car actions) 'primitive)) #t)
;          ((and (eq? names exp) (not (eq? (car actions) 'primitive))) #f)
;          (else
;           (scan (cdr names) (cdr actions)))))
;  (scan (car the-global-environment) (cdr the-global-environment)))
          

;(define (install-primitive-procedure name action)
;  (cond ((special-form-exist? name)
;         (display name)
;         (display " already exists as a special form.")
;         (newline))
;        ((primitive-proc-exist? name)
;         (display name)
;         (display " already exists as a primitive procedure.")
;         )
;        (else
...
(define (install-primitive-procedure name action)
  (define (install-primitive-procedure-global name action env)
    (define (scan names actions)
      (cond ((null? names)
             (add-binding-to-frame! name (list 'primitive action) env)
             ;(display name)
             ;(newline)
             )
            ((eq? name (car names))
             (display "No can do. Somethings here")
             (newline))
            (else (scan (cdr names) (cdr actions)))))
    (scan (car env)
          (cdr env)))
  (install-primitive-procedure-global name action (car the-global-environment)))
