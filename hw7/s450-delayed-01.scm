...
(define (xapply procedure arguments)
  (cond ((primitive-procedure? procedure)
        ...
        (else
         ...
         (s450error "Unknown procedure type -- XAPPLY  " procedure)
         )))
  ...
  (if (not (lookup-action (cadr exp)))
      (let ((name (assignment-variable exp)))
        (set-variable-value! name
                             ...
                             env)
        name)
      "ERROR--reassigning a special form name"))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;	 Representing Delayed Expressions
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;	 Representing Dynamic Expressions
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (dynamic? obj)
  (tagged-list? obj 'dynamic))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;	 Representing Reference Expressions
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ...
      (let ((first (car clauses))
        ...
        (if (cond-else-clause? first)
            (if (null? rest)
                ...
                (s450error "ELSE clause isn't last -- COND->IF " clauses)
                )
...
(define (install-special-form key value)
  (if (member key (caar the-global-environment))
      (display "ERROR--giving a defined name to a new special form")
      (cond ((not (lookup-action key))
             (insert! key value special-form-table)
             (newline))
            (else (display "ERROR--reinstalling a special form")
                  (newline)))))
...
; SPECIAL FORM 'locally-defined?' returns #t iff the <symbol> is
;               defined in the first frame of the current environment.
(define (locally-defined? var env)
  (define (scan vars)
    (cond ((null? vars)
           #f)
          ((equal? var (car vars))
...
; SPECIAL FORM 'locally-make-unbound!' removes the <symbol> binding(s) from the
  ...
  (define (install-primitive-procedure-global name action env)
    (let ((frame (first-frame env)))
      (define (scan names actions)
        (cond ((null? names)
               ; Check primitive proc name in SFT
               (if (not (lookup-action name))
                   ; if its not there, go ahead and install into G.E.
                   (add-binding-to-frame! name (tag-primitive action) frame)
                   ; if its there, can't install it
                   (display "ERROR--installing a primitive procedure with the name of a special form")))
              ;Trying to install a primitive procedure with name of proc that already exists
              ((eq? name (car names))
...
; Set global var exit? to true so program knows not to loop
(define (exit)
  (set! exit? #t)
  (display "Leaving S450"))

; display error message and lists the arguments applied that failed
(define (s450error msg . args)
  (newline)
  (display msg)
...
(define (user-print object)
  (cond ((user-defined-procedure? object)
         (display (list 'user-defined-procedure
                     ...
                     (procedure-body object)
                     '<procedure-env>)))
        ((eq? (if #f #t) object) (display "")) ; void display nothing
        (else
         (display object))))
...
(install-special-form 'defined? (lambda (exp env) (defined? (cadr exp) env)))
(install-special-form 'locally-defined? (lambda (exp env) (locally-defined? (cadr exp) env)))
(install-special-form 'make-unbound! (lambda (exp env) (make-unbound! (cadr exp) env)))
(install-special-form 'locally-make-unbound! (lambda (exp env) (locally-make-unbound! (cadr exp) env)))
