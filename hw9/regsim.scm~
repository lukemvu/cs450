...
(define (make-new-machine)
    (let ((pc (make-register))
      ...
      (let ((the-ops
        ...
        (define (execute)
          ;; modified with an additional check of instruction in breakpoint list
          (let* ((insts (get-contents pc))
                 ;; ran into issue with trying to access (car insts) of empty insts
                 ;; that's what this crazy if statement in here checks for
                 (breakpoint (assoc (if (null? insts) #f (car insts)) breakpoint-list)))            
            (cond ((null? insts)
                   'done)
                  ;; Check if the instruction exists in the breakpoint list
                  ((not (equal? #f breakpoint))
                   (display "Stopped at breakpoint: ")
                   (display (cadr breakpoint))
                   (display " ")
                   (display (cddr breakpoint))
                   (newline))
                  (else
                   (begin
                     ((instruction-execution-proc (car insts)))
                     (execute))))))
        ;; proceed-machine works exactly like the original execute
        ...
        (define (set-breakpoint label n)
          ;; Find list of instructions bound to specified label
          (define (find-label label instruction-labels)
            (if (equal? label (caar instruction-labels))
                (car instruction-labels)
                (find-label label (cdr instruction-labels))))
          ;; Find specific instruction at n offset from label
          (define (find-instruction n instruction-set)
            (if (equal? n 1)
                (cadr instruction-set)
                (find-instruction (- n 1) (cdr instruction-set))))
          ;; Breakpoint-list is a list of pairs of pairs
          ;; One pair is the instruction-pair (as found in the instruction sequence)
          ;; Second pair is the label and offset n
          (let* ((instructions (find-label label labels))
                 (instruction (find-instruction n instructions)))              
            (set! breakpoint-list
                  (cons (cons instruction (cons label n)) breakpoint-list))))
        ;; Remove breakpoints
        (define (cancel-breakpoint label n)
          (define (scan prev-bp-list bp-list)
            (cond ((and (equal? label (cadar bp-list))
                        (equal? n (cddar bp-list)))
                   (set-cdr! prev-bp-list (cdr bp-list))
                   ...
                   (newline))
                  (else
                   (scan bp-list (cdr bp-list)))))
          ;; Case where first breakpoint needs to be removed
          (if (and (equal? label (cadar breakpoint-list))
                   (equal? n (cddar breakpoint-list)))
              (set! breakpoint-list (cdr breakpoint-list))
              (scan breakpoint-list (cdr breakpoint-list))))        
        ;; Cancel all breakpoints
        (define (cancel-all-breakpoints)
          ;; Set the break-point list to the empty set
          (set! breakpoint-list '())
          (display "All breakpoints removed.")
          (newline))
        ;; Add labels from extract-label to local machine variable
        (define (insert-instruction-labels the-labels)
          (set! labels the-labels))
        ;; Testing functions
        (define (print-breakpoints)
          (for-each (lambda (x) (display x) (newline)) breakpoint-list))
        (define (print-instructions)
          (for-each (lambda (x) (display x) (newline)) the-instruction-sequence))
        (define (print-instruction-labels)
          (for-each (lambda (x) (display x) (newline) (newline)) labels))
        ;;BREAKPOINT RELATED FUNCTIONS END
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ((eq? message 'insert-instruction-labels) insert-instruction-labels)
                ((eq? message 'proceed-machine) proceed-machine)
                ((eq? message 'cancel-breakpoint) cancel-breakpoint)
                ;; Testing functions
                ((eq? message 'print-breakpoints) print-breakpoints)
                ((eq? message 'print-instructions) print-instructions)
                ((eq? message 'print-instruction-labels) print-instruction-labels)
...
;; Testing Functions
(define (print-breakpoints machine)
  ((machine 'print-breakpoints)))

(define (print-instructions machine)
  ((machine 'print-instructions)))

(define (print-instruction-labels machine)
  ((machine 'print-instruction-labels)))
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
                    ...
                    ((machine 'insert-instruction-labels) labels)
                    insts)))
      ...
      (extract-labels
       ...
       (lambda (insts labels)             ;; This is where labels and insts are
         (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
           (if (symbol? next-inst)
               (if (not (assoc next-inst labels)) ;; Check if label exists in label table
                   (receive insts                              ;; either like this
                            (cons (make-label-entry next-inst  ;;  (it's a label)
                                                    insts)
                                  labels))
                   (error "ERROR -- Label already exists: " next-inst))
